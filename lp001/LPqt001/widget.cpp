...
//镜像
void mirrorY(cv::Mat src, cv::Mat& dst)
{
    int row = src.rows;
    int col = src.cols;
    dst = src.clone();
...
//    connect(ui->human,&QPushButton::clicked,this,&Widget::HumanFea);
    ...
    connect(ui->handButton,&QPushButton::clicked,this,&Widget::Hand);
...
cv::Mat Widget::test(cv::Mat &img){

   cv::Mat frame=img;
   cv::Mat mu[5];
   mu[0] = cv::imread("m1.png", CV_8UC1); // 手势1
   mu[1] = cv::imread("m2.png", CV_8UC1); // 手势2
   mu[2] = cv::imread("m3.png", CV_8UC1); // 手势3
   mu[3] = cv::imread("m4.png", CV_8UC1); // 手势4
   mu[4] = cv::imread("m5.png", CV_8UC1); // 手势5


   string str1, str2, str3;
   cv::Mat binImage, tmp, tmp1;
   cv::Mat Y, Cr, Cb, H;
...
//   int xg_num = 0;
   ...
   mirrorY(frame, frame);
           ...
           // 颜色空间变换（RGB to GRAY）
   cvtColor(frame, binImage, cv::COLOR_BGR2GRAY);

           // 颜色空间变换（RGB to YCrCb）
   frame.copyTo(tmp);
   cvtColor(tmp, tmp, cv::COLOR_BGR2YCrCb);

           // 颜色空间变换（RGB to HSV）
   frame.copyTo(tmp1);
   cvtColor(tmp1, tmp1, cv::COLOR_BGR2HSV);
   split(tmp, channels);
   ...
   for (int j = 1; j < Cr.rows - 1; j++)	// 遍历图像像素点
     ...
     // 灰度形态学处理
             erode(binImage, binImage, cv::Mat());
             dilate(binImage, binImage, cv::Mat());
             ...
             for (int i = 0; i < label; i++)
             {
                 ...
                 {
                     // 尺度调整
                     cv::Mat rROI =cv:: Mat(dsize, CV_8UC1);
                     cv::resize(waterShed(cv::Rect(xmin[i], ymin[i], xmax[i] - xmin[i], ymax[i] - ymin[i])), rROI, dsize);
                     /*imshow("手势区域", rROI);*/
                    ...
                    cv:: Mat result;
                     ...
                     putText(waterShed, str1, end, cv::FONT_HERSHEY_DUPLEX, 0.7, cv::Scalar(255, 0, 0), 2);
                     rectangle(waterShed, cv::Point(xmin[i], ymin[i]),cv:: Point(xmax[i], ymax[i]), cv::Scalar::all(255), 3, 8, 0);	// 加框
                     rectangle(frame,cv:: Point(xmin[i], ymin[i]),cv:: Point(xmax[i], ymax[i]), cv::Scalar(0, 255, 0), 3, 8, 0);	// 加框
                 }
                     }
             return frame;
}
...
void Widget::slotDisImg(QImage &img)
{
    ...
    }else if (this->demoo==3) {
        newimg=this->test(newimg);
